...
class HomeController extends Controller
{
    ...
    public function HomePageByLang($lang = "")
    {
        ...
        // General Webmaster Settings
        $WebmasterSettings = WebmasterSetting::find(1);
        $WebmasterSection = WebmasterSection::where('title_en', 'Opinion')->first();
        $sectionVide = WebmasterSection::where('title_en', 'Videos')->first();
        // Home topics
        ...
        // Home Opinion
        $HomeOpinions = Topic::where([['status', 1], ['webmaster_id', $WebmasterSection->id]])->get();
        $HomeVides = Topic::where([['status', 1], ['webmaster_id', $sectionVide->id]])->get();
        // Home photos
        $HomePhotos = Topic::where([['status', 1], ['webmaster_id', $WebmasterSection->home_content2_section_id], ['expire_date', '>=', date("Y-m-d")], ['expire_date', '<>', null]])->orwhere([['status', 1], ['webmaster_id', $WebmasterSettings->home_content2_section_id], ['expire_date', null]])->orderby('date', env("FRONTEND_TOPICS_ORDER", "asc"))->limit(6)->get();
        // Home Partners
        ...
        if (!empty($WebmasterSection)) {
            ...
            if (!empty($CurrentCategory)) {
                ...
                $Topics = Topic::where([['webmaster_id', '=', $WebmasterSection->id], [
                    'status',
                    1
                ], ['expire_date', '>=', date("Y-m-d")], ['expire_date', '<>', null]])->orWhere([['webmaster_id', '=', $WebmasterSection->id], ['status', 1], ['expire_date', null]])->orderby('date', env("FRONTEND_TOPICS_ORDER", "asc"))->paginate(env('FRONTEND_PAGINATION'));
                // Get Most Viewed
    ...
    public function FrontendGalleryByLang($lang = "", $section = 0, $id = 0)
    {
...
<p>$site_msg</p>
    ...
    public function lifeMember(LifeMemberRequest $request)
    {
    ...
    public function allDepartments($lang = "", $section = "sitepages", $id = 26)
    {
    ...
    public function teacher($lang = "bd", $section = 'amader-smprke', $id = 27)
    {
        // General Webmaster Settings
        $WebmasterSettings = WebmasterSetting::find(1);
        ...
        if (!empty($WebmasterSection)) {
            ...
            // .. end of .. Page Title, Description, Keywords
            //Committee data
            $teachers = TeacherRepository::query()->where('is_active', true)->get();
    ...
    public function departmentStudent($departmentId)
    {
        ...
        if (!empty($Topic) && ($Topic->expire_date == '' || ($Topic->expire_date != '' && $Topic->expire_date >= date("Y-m-d")))) {
            ...
            if (!empty($WebmasterSection)) {
                ...
                // .. end of .. Page Title, Description, Keywords
                $students = StudentRepository::query()->where('is_active', true)->where('department_id', $departmentId)->orderby('roll', 'asc')->get();
                $department = DepartmentRepository::query()->where('id', $departmentId)->first();
