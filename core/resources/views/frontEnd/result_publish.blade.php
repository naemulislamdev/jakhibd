...
<body>
    <section>
        <div class="container">
            <div class="row">
                <div class="col-md-6">
                    <span class="text-center"><a href="{{ route('Home') }}" role="button" style=""
                            class="btn btn-danger pull-left active"> পূর্বের
                            পেইজ যান </a></span>
                </div>
                <div class="col-md-6">
                    <span>

                        <span style="float: right;"><a href="#" role="button" id="printResultBtn"
                                class="btn btn-primary pull-right active">
                                প্রিন্ট করুন </a></span></span>
                </div>
            </div>
            <div class="row">
                                    ...
                                    <tbody>
                                        <tr>
                                            <td colspan="4" class="heading-row">শ্রেণি .
                                                {{ $results->first()->student->department->name }}</td>
                                        </tr>
                                        <tr style="width: 100%">
                                            <td colspan="2">রোল নং:
                                                {{ $numto->bnNum($results->first()->student->roll) }}</td>
                                            <td colspan="1">সিট নং:
                                        ...
                                        <tr>
                                            <td style="" colspan="2">নাম:
                                                {{ $results->first()->student->name }}</td>
                                            <td colspan="1">জন্ম তারিখ:
                                                {{ $results->first()->student->date_of_birth }}</td>

                                        </tr>
                                        <tr>
                                            <td colspan="4">পিতার নাম: {{ $results->first()->student->father_name }}
                                            </td>
                                        </tr>
                                        <tr>
                                            <td colspan="4">মাদরাসার নাম: জামিয়া আরাবিয়া খদিমুল ইসলাম মাদরাসা ও
                                                এতিমখানা</td>
                                        </tr>
                                        <tr>
                                            <td colspan="4">ঠিকানা: ব্লক-এ (টিনশেড কলোনী), সেকশন-১৩, থানা: কাফরুল,
                                                মিরপুর, ঢাকা-১২১৬।</td>
                                        </tr>
                                        ...
                                        @foreach ($results as $result)
                                            <tr style="width: 100%;">
                                                <td style="width:20%; text-align:center;">
                                                    {{ $numto->bnNum($loop->index + 1) }}</td>
                                                <td style="width:45%">{{ $result->subject->name }}</td>
                                                <td style="width:30%; text-align:center;">
                                                    {{ $numto->bnNum($result->mark) }}</td>
                                            </tr>
                                        @endforeach
                                        <tr>
                                            <th style=" text-align:right;" colspan="2">মোট প্রাপ্ত নম্বর</th>
                                            <th style="width:50%; text-align:center;">{{ $numto->bnNum($total_marks) }}
                                            </th>
                                        </tr>
                                    </tbody>
                                </table>
