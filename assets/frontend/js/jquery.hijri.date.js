...
$.fn.hijriDate = function (options = {}) {
  ...
  const hijriMonths = {
    ar: ["মুহাররম", "সফর", "রবিউল আউয়াল", "রবিউস সানি", "জুমাদিউল উলা", "জুমাদিউল উখরা", "রজব", "শাবান", "রমজান", "শাওয়াল", "জিলক্বদ", "জিলহজ্ব"],
  ...
  const hijriSymbol = (defaultOptions.hijriLang === 'ar') ? 'হিঃ' : '';
  ...
  const gregDate = defaultOptions.showGregDate ?
    `${defaultOptions.separator} <span class="greg-date"> ${gregMonths[defaultOptions.gregLang][m]} ${y} ${gregSymbol}</span>` : '';
...
//   const weekDay = defaultOptions.showWeekDay ? weekDays[defaultOptions.weekDayLang][dow] : '';
  ...
  const printedDate = `<span class="hijri-date">${hijriDate}</span> ${gregDate}`;

  $hostEl.html(printedDate.convertDigitToBangla());
}
